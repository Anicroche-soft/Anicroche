const erreur = (message, str, pos) =>
{
    ...
    throw new Error(`Analyseur .avec : ${ligne}:${colonne} : ${message}`)
}
...
const valider_bloc_avec = (bloc, str, pos) =>
{
    if (bloc.type == 'instruction')
    {
        if (bloc.args[0] == `@style` || bloc.args[0] == `@script`)
        {
            if (bloc.args.length != 2)
                erreur(`${bloc.args[0]} doit avoir un argument (${bloc.args[0]} <texte>)`, str, pos)
            if (bloc.enfants.length > 0)
                erreur(`${bloc.args[0]} ne doit pas avoir d'enfant`, str, pos)
        }
        else if (bloc.args[0] == `@if` || bloc.args[0] == `@else-if` || bloc.args[0] == `@unless` || bloc.args[0] == `@while` || bloc.args[0] == `@until`)
        {
            if (bloc.args.length != 2)
                erreur(`${bloc.args[0]} doit avoir un argument (${bloc.args[0]} <condition>)`, str, pos)
            if (bloc.enfants.length <= 0)
                erreur(`${bloc.args[0]} doit avoir au moins un enfant`, str, pos)
        }
        else if (bloc.args[0] == `@else`)
        {
            if (bloc.args.length != 1)
                erreur(`${bloc.args[0]} ne doit pas avoir d'argument (${bloc.args[0]})`, str, pos)
            if (bloc.enfants.length <= 0)
                erreur(`${bloc.args[0]} doit avoir au moins un enfant`, str, pos)
        }
        ...
        {
            if (bloc.args.length > 2)
                erreur(`${bloc.args[0]} peut avoir un argument (${bloc.args[0]} [<nombre>])`, str, pos)
            if (bloc.enfants.length <= 0)
                erreur(`${bloc.args[0]} doit avoir au moins un enfant`, str, pos)
        }
        else if (bloc.args[0] == `@for`)
        {
            if (bloc.args.length != 4 || (bloc.args[2] != 'in' && bloc.args[0] != 'of'))
                erreur(`${bloc.args[0]} doit avoir trois arguments (${bloc.args[0]} <variable> (in | of) <variable>)`, str, pos)
            if (bloc.enfants.length <= 0)
                erreur(`${bloc.args[0]} doit avoir au moins un enfant`, str, pos)
        }
        else if (bloc.args[0] == `@break` || bloc.args[0] == `@continue` || bloc.args[0] == `@stud`)
        {
            ...
            erreur(`${bloc.args[0]} n'est pas reconnu comme instruction`, str, pos)
    }
    else if (bloc.type == 'balise')
    {
        ...
        if (bloc.args.length != 1)
            erreur(`<${balise}> ne peut pas avoir d'argument`, str, pos)
        if (balises_interdites.includes(balise))
            erreur(`<${balise}> est une balise interdite`, str, pos)
        if (balises_sans_enfant.includes(balise) && bloc.enfants.length > 0)
            erreur(`<${balise}> ne peut pas avoir d'enfants`, str, pos)
    }
    else if (bloc.type == 'texte')
    {
        if (bloc.args.length != 1)
            erreur(`un texte ne peut pas avoir d'argument`, str, pos)
        if (bloc.enfants.length > 0)
    ...
    else if (bloc.type == 'modele')
    {
        ...
        erreur("Syntaxe incorrecte", str, pos)
}

const analyser_bloc_avec = (str, deb, fin, json) =>
{
    ...
    if (str[pos] == '\t')
        return erreur("Indentation par tabulations interdite", str, pos)
    if (str[pos] == '\n' || str[pos] == '#')
        return null
    ...
    while (!fini)
    {
        ...
        while (!/^[ \n]$/.test(str[pos]) || blocs != '')
        {
            ...
            {
                if (blocs != '')
                    return erreur("Fermeture de bloc manquante", str, fin + 1)
                fini = true
                break
            }
            pos++
    ...
    valider_bloc_avec(bloc, str, deb + indentation)
    return bloc
}

const analyser_enfants_avec = (str, deb, fin, json) =>
{
    ...
    while (!fini)
    {
        ...
        for (let i = 0; i < indentation && pos + i < fin; i++)
        {
            ...
            if (str[pos + i] == '\t')
                return erreur("Indentation par tabulations interdite", str, pos + i)
            if (str[pos + i] != ' ')
                return erreur("Erreur d'indentation", str, pos + i)
        }
        if (/^[\n#]$/.test(str[pos]))
        {
            while(str[pos] != '\n' && pos < fin)
        ...
        if (str[pos] == '\t')
            return erreur("Indentation par tabulations interdite", str, pos)
        if (str[pos] == ' ')
            return erreur("Erreur d'indentation", str, pos)
        ...
        let bloc = analyser_bloc_avec(str, deb_enf, fin_enf, json + 1)
        if (bloc)
            enfants.push(bloc)
        pos = fin_enf + 1
    }
